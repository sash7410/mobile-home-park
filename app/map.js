...
// Main component for displaying the interactive map
const Map = () => {
  ...
  // Function to generate filter options based on park attributes
  const getFilterOptions = parks => {
    ...
    return [
      ...Array.from(sizeOptions).map(size => ({ value: size, label: size[0].toUpperCase() + size.slice(1) })),
      ...Array.from(amenityOptions).map(amenity => ({ value: amenity, label: amenity[0].toUpperCase() + amenity.slice(1) }))
    ];
  };
  ...
  useEffect(() => {
    ...
    loader.load().then(() => {
      ...
      initialParks.forEach(park => createMarker(map, park));
    }).catch(error => console.error('Error loading the Google Maps API', error));
  }, []);

  // Function to create a marker for a park
  const createMarker = (map, park) => {
    ...
    const infoWindow = new google.maps.InfoWindow({
      content: `
        <div style="width: 280px; padding: 15px; background: white; border: 1px solid #ddd; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
  ...
  // Update markers based on search and filter options
  useEffect(() => {
  ...
  const handleFilterChange = options => setSelectedOptions(options);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <Select
        isMulti
        options={getFilterOptions(parks)}
